...
The original brief called for monitoring Facebook Marketplace, but automating that via browser drivers violates Meta's Terms of Service, so that surface was intentionally left out. Instead, the project leans on an **Adapter Pattern**: a first-party Web Chat channel (over WebSocket) is the live, publicly demonstrable surface, and a `MockFacebookAdapter` serves as a deterministic test harness proving the same agent logic generalizes to other channels — a compliant Facebook Graph API integration could be dropped in later without touching agent logic.
